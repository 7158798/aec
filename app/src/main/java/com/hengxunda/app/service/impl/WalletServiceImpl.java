...
public class WalletServiceImpl implements IWalletService {
    ...
    public void transfer(TradeDto tradeDto) {
        ...
        String userId = shiroUserPo.getId();
        String coinType = tradeDto.getCoinType();
        String payType = tradeDto.getPayType();

        WalletInfo fromWalletInfo = walletInfoCustomMapper.getByUserIdAndType(userId, coinType);
        WalletInfo toWalletInfo = walletInfoCustomMapper.getByAddress(tradeDto.getToAddress(), coinType);

        A.check(fromWalletInfo.getAddress().equals(toWalletInfo.getAddress()), "不允许自己与自己交易!");
        A.check(!StringUtils.equals(tradeDto.getFromAddress(), fromWalletInfo.getAddress()), "请认真核对地址信息!");
        A.check(!StringUtils.equals(tradeDto.getToAddress(), toWalletInfo.getAddress()), "请认真核对地址信息!");
        checkWalletAddressIsNotExist(fromWalletInfo);
        checkWalletAddressIsNotExist(toWalletInfo);
        checkBalance(tradeDto.getAmount(), fromWalletInfo.getBalance());
        checkEthIsNotError(tradeDto.getFromAddress());
        checkEthIsNotError(tradeDto.getToAddress());
        ...
        if (PayWayEnum.AEC_PAY.getCode().equals(payType)) {
        ...
        } else if (PayWayEnum.MERCHANT_AEC_PAY.getCode().equals(payType)) {
            //AEC商家收款
            setWalletRecord(walletRecord, tradeDto, TransPairEnum.AEC2AEC.getCode(), TransPairEnum.AEC2AEC.getValue());
        } else if (PayWayEnum.MERCHANT_MSC_PAY.getCode().equals(payType)) {
            //MSC商家收款
            setWalletRecord(walletRecord, tradeDto, TransPairEnum.MSC2MSC.getCode(), TransPairEnum.MSC2MSC.getValue());
        }

        //转出装入钱包金额
        int updateCount = walletInfoCustomMapper.updateSameCoinTradeBalance(walletRecord);
        A.check(updateCount != 2, "可用余额不足!");
        ...
        if (StringUtils.isNoneBlank(orderId) && orderId.length() > 0) {
            int updateCount1 = tradeOutLogCustomMapper.updateByBillCode(orderId, "1");
            A.check(updateCount1 != 1, "订单支付失败！");
        }
    }
    ...
    public OrderInfoVo orderInfo(String orderId) {
        TradeOutLog tradeOutLog = tradeOutLogCustomMapper.selectByBillCode(orderId);
        A.check(Objects.isNull(tradeOutLog), "订单不存在!");
        A.check("1".equals(tradeOutLog.getStatus()), "该订单已支付完成，请勿重复支付");

        WalletInfo walletInfo = walletInfoCustomMapper.getByUserIdAndType(tradeOutLog.getShopuserid(), tradeOutLog.getCointype());
        A.check(Objects.isNull(walletInfo), "钱包信息不存在！");

        OrderInfoVo orderInfoVo = new OrderInfoVo();
        orderInfoVo.setOrderId(orderId)
                .setCoinType(tradeOutLog.getCointype())
                .setAddress(walletInfo.getAddress())
                .setAmount(tradeOutLog.getAmount());
        return orderInfoVo;
    }
